...
const ICON_CODE_DESCRIPTIONS: Record<string, string> = {
  '00': 'Sunny',
  '01': 'A few clouds',
  '02': 'A mix of sun and cloud',
  '03': 'Cloudy periods',
  '04': 'Increasing cloudiness',
  '05': 'Clearing',
  '06': 'Chance of showers',
  '07': 'Chance of flurries or rain showers',
  '08': 'A few flurries',
  '09': 'Chance of thunderstorms',
  '10': 'Cloudy',
  '11': 'Overcast',
  '12': 'Showers',
  '13': 'Periods of rain',
  '14': 'Chance of freezing rain',
  '15': 'Rain or snow',
  '16': 'Flurries',
  '17': 'Periods of snow',
  '18': 'Blizzard',
  '19': 'Showers or thunderstorms',
  '20': 'Fog',
  '21': 'Fog patches',
  '22': 'Smoke',
  '23': 'Dust',
...
export async function fetchWeather(
  ...
  if (!filename)
    throw new Error(`Weather file not found for ${siteCode} in ${province}`)
  const h = hour.toString().padStart(2, '0')
  ...
  const windGust =
    windGustRaw === '00' || windGustRaw === ''
      ? windSpeed
      : (num(windGustRaw) ?? windSpeed)
  const windBearing = num(extractText(windObj?.bearing))
  const windDirection =
    windBearing != null && windBearing >= 0 && windBearing <= 360
      ? windBearing
      : null
  ...
  for (let i = 0; i < forecastList.length; i++) {
    ...
    const tempArr = Array.isArray(tempList)
      ? tempList
      : tempList
        ? [tempList]
        : []
    const highTemp = tempArr.find(
      (t: Record<string, unknown>) =>
        (t as Record<string, string>)['@_class'] === 'high',
    )
    const lowTemp = tempArr.find(
      (t: Record<string, unknown>) =>
        (t as Record<string, string>)['@_class'] === 'low',
    )
    ...
    const isDayPeriod =
      !textForecastName.toLowerCase().includes('night') &&
    ...
    if (isDayPeriod && dayForecasts.length < 7) {
      ...
      const nextTemps = nextF?.temperatures as
        | Record<string, unknown>
        | undefined
      const nextTempList = nextTemps?.temperature
      const nextTempArr = Array.isArray(nextTempList)
        ? nextTempList
        : nextTempList
          ? [nextTempList]
          : []
      const nextLow = nextTempArr.find(
        (t: Record<string, unknown>) =>
          (t as Record<string, string>)['@_class'] === 'low',
      )
      ...
      dayForecasts.push({
        ...
        condition:
          textSummary ||
          cloudPrecipSummary ||
          ICON_CODE_DESCRIPTIONS[icon ? String(icon).padStart(2, '0') : '00'] ||
          '',
  ...
  let todayHighTime: string | null = null
  const hourlyGroup = siteData?.hourlyForecastGroup as
    | Record<string, unknown>
    | undefined
  const forecastIssueLocal = (hourlyGroup ?? forecastGroup)?.dateTime
  const issueDateTimes = Array.isArray(forecastIssueLocal)
    ? forecastIssueLocal
    : forecastIssueLocal
      ? [forecastIssueLocal]
      : []
  const issueMst =
    issueDateTimes.find((dt: Record<string, unknown>) =>
      String((dt as Record<string, string>)['@_zone'] ?? '').match(
        /^(MST|MDT|America\/Edmonton)$/i,
      ),
    ) ?? issueDateTimes[0]
  const issueYear = num(extractText(issueMst?.year))
  const issueMonth = num(extractText(issueMst?.month))
  const issueDay = num(extractText(issueMst?.day))
  const todayLocal =
    issueYear != null && issueMonth != null && issueDay != null
      ? `${issueYear}-${String(issueMonth).padStart(2, '0')}-${String(issueDay).padStart(2, '0')}`
      : null

  const hourlyList = hourlyGroup?.hourlyForecast
  const hourlies = Array.isArray(hourlyList)
    ? hourlyList
    : hourlyList
      ? [hourlyList]
      : []
  const parsedHourlies: HourlyForecast[] = []
  for (const h of hourlies) {
    const ho = h as Record<string, unknown>
    const dtUtc = (ho['@_dateTimeUTC'] ?? ho['dateTimeUTC']) as
      | string
      | undefined
    if (!dtUtc || dtUtc.length < 12) continue
    const y = parseInt(dtUtc.slice(0, 4), 10)
    const m = parseInt(dtUtc.slice(4, 6), 10) - 1
    const d = parseInt(dtUtc.slice(6, 8), 10)
    const hr = parseInt(dtUtc.slice(8, 10), 10)
    const min = parseInt(dtUtc.slice(10, 12), 10) || 0
    const utcDate = new Date(Date.UTC(y, m, d, hr, min))
    const temp = roundTemp(num(extractText(ho?.temperature)))
    if (temp == null) continue
    const windObj = ho?.wind as Record<string, unknown> | undefined
    const speed = num(extractText(windObj?.speed)) ?? 0
    const gustRaw = extractText(windObj?.gust)
    const gust =
      gustRaw === '00' || gustRaw === '' ? speed : (num(gustRaw) ?? speed)
    const bearing = num(extractText(windObj?.bearing))
    const dir =
  ...
  if (todayLocal) {
    const todayHours = parsedHourlies.filter(
      (entry) =>
        entry.utc.toLocaleDateString('en-CA', { timeZone: CANMORE_TZ }) ===
        todayLocal,
    )
    const maxEntry = todayHours.reduce(
      (best, cur) =>
        cur.temp > best.temp ||
        (cur.temp === best.temp && cur.utc.getTime() < best.utc.getTime())
          ? cur
          : best,
      {
        utc: new Date(0),
    ...
    if (maxEntry.temp === todayHigh) {
      todayHighTime = maxEntry.utc.toISOString()
    }
  }
